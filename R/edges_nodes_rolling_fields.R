...
normalize_size <- function(x, min_size = 5, max_size = 100) {
...
for (i in seq_len(nrow(windows))) {
  ...
  # Skip empty graphs
  if (vcount(g) == 0 | ecount(g) == 0) next
  
  # --- Assign stable colors and sizes ---
  V(g)$color   <- field_colors[V(g)$field_1]
  V(g)$cluster <- as.integer(factor(V(g)$field_1, levels = all_fields))
  V(g)$size <- normalize_size(V(g)$cited_by_count)
  V(g)$label   <- V(g)$title
  ...
  nodes_json <- tibble(
    ...
    color = V(g)$color,
    cluster = V(g)$cluster,
    date = V(g)$publication_year,
    authors = V(g)$authors,
    citations = V(g)$cited_by_count,
    domain = V(g)$domain_1,
    field = V(g)$field_1,
    window = V(g)$window
  ...
  edges_json <- tibble(
    id = paste0("e", seq_len(ecount(g))),
    source = as.character(ends(g, E(g))[,1]),
    target = as.character(ends(g, E(g))[,2]),
    weight = E(g)$weight
  )
  ...
  write(
    ...
    file = sprintf("sigma_network/rolling_network/public/windows_field/nodes_window_%03d.json", i)
  )
  
  write(
    toJSON(edges_json, dataframe = "rows", auto_unbox = TRUE, pretty = TRUE),
    file = sprintf("sigma_network/rolling_network/public/windows_field/edges_window_%03d.json", i)
  )
  
  message("Saved window ", i, " (", w$start, "-", w$end, ") with ", vcount(g), " nodes and ", ecount(g), " edges.")
}

field_color <- data.frame(field = all_fields, color = field_colors)

jsonlite::write_json(field_color, "sigma_network/rolling_network/public/field_color.json", pretty = TRUE)
